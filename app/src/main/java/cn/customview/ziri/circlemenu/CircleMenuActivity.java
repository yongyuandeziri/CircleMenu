package cn.customview.ziri.circlemenu;
...
public class CircleMenuActivity extends AppCompatActivity  implements View.OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.icon_menu:	//处理 menu 图标的点击事件
                ...
                if(isLevel3Show){
                    //隐藏 第3级菜单
                    AnimationUtils.startAnimOut(level3);
                }else{
                    // 如果第3级菜单是隐藏状态，那么将其显示
                    AnimationUtils.startAnimIn(level3);
                }
            ...
            case R.id.icon_home:	//处理 home 图标 的点击事件
                ...
                if(isLevel2Show ){
                    AnimationUtils.startAnimOut(level2);
                    isLevel2Show = false;

                    if(isLevel3Show){ // 如果此时，第3级菜单也显示，那也将其隐藏
                        AnimationUtils.startAnimOut(level3,200);
                        isLevel3Show = false;
                    }

                }else{
                    // 如果第2级菜单是隐藏状态，那么就显示2级菜单
                    AnimationUtils.startAnimIn(level2);
                    isLevel2Show = true;
                }
    ...
    private void changeLevel1State() {
        ...
        if(isLevel1show){
            AnimationUtils.startAnimOut(level1);
            isLevel1show = false;

            if(isLevel2Show){ // 判断2级菜单是否显示
                AnimationUtils.startAnimOut(level2,100);
                isLevel2Show = false;
            ...
            AnimationUtils.startAnimIn(level1);
            isLevel1show = true;

            AnimationUtils.startAnimIn(level2,200);
            isLevel2Show = true;
